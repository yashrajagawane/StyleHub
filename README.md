...
### P1 (next iteration)
- **Cloudinary image upload** — currently image URLs are entered manually
- **CSV / XLSX import & export** for bulk catalogue ops
- **Suppliers module** — track vendors, PO history, purchase price vs. selling price
- **Audit log page** — surface the existing `stock_history` collection + admin edits

### P2 (future)
- Refresh-token rotation with HttpOnly cookie + CSRF token
- Docker Compose + Nginx bundle for self-hosting
- Product comparison table
- QR code generation on product detail (for in-store scanning)
- Server-side "recently viewed" per authenticated staff
- Sitemap.xml, product-level Open Graph tags
- **"Reserve & Try" appointment slots** — customer picks a time, boutique gets WhatsApp with SKU + slot

### P3 (nice-to-have)
- Multi-store / multi-location support
- Emergent Google OAuth for staff (owner / stylist / buyer roles)
- Next.js SSR migration (if SEO becomes a bigger priority)

---

## FAQ

**Q: Why no cart / checkout?**
A: This is intentional. StyleHub is a **digital showroom** for a physical boutique. The goal is to
   drive foot traffic and reservations, not compete with the store experience. Every product page
   has WhatsApp / Call / Visit buttons instead.

**Q: Where does the wishlist live?**
A: In the browser's `localStorage`. No login is required — customers can save pieces anonymously.

**Q: Why MongoDB instead of PostgreSQL?**
A: The Emergent preview environment ships with MongoDB and hot reload. The models are flat
   documents with UUID primary keys; migration to Postgres would only require replacing the
   Motor queries with SQLAlchemy — no schema changes.

**Q: Can I change currency?**
A: Yes. Edit `frontend/src/lib/format.js` — change `"en-IN"` to your locale and swap the `₹`
   prefix. Everywhere in the app calls `formatINR()`, so it's a single-file change.

**Q: How do I add a new product?**
A: Log in as admin → `/admin/products` → **New Product** → fill the modal → save. Images accept
   comma-separated URLs and preview inline.

**Q: How do I change the store name, phone, WhatsApp number, or business hours?**
A: `/admin/settings` — everything is editable there. Changes propagate to the footer, contact
   page, home page, and every WhatsApp CTA immediately.

**Q: Where does the "Stock Value" figure on the dashboard come from?**
A: Backend sums `price × stock` across all products. Purely wholesale-value indicative.

---

## License

Proprietary — © StyleHub Boutique 2026. All rights reserved.
Image assets are sourced from Unsplash under their license and remain the property of their
...
Built with 🖤 on the **Emergent** full-stack platform.
Typography: [Playfair Display](https://fonts.google.com/specimen/Playfair+Display) & [Manrope](https://fonts.google.com/specimen/Manrope).
